...
# Bản đồ giảm nhạy cảm tiếng Trung (Censor Bypass) để tránh bị Gemini / LLM Blocked Content
CENSOR_BYPASS_MAP = {
    "水乳交融": "完美融合",
    "水乳、交融": "完美、融合",
    "乳交": "乳.交",
...
def preprocess_chinese_text(raw_text: str) -> str:
    ...
    Bước 0: Giảm nhạy cảm các cụm từ dễ kích hoạt bộ lọc an toàn của LLM
    ...
    # Bước 0: Censor Bypass
    for old, new in CENSOR_BYPASS_MAP.items():
