...
class GenericScraper(BaseScraper):
    ...
    async def _fetch_html_static(self, url: str) -> str:
        """Fetches static HTML using httpx with smart encoding detection."""
        async with httpx.AsyncClient(timeout=20.0, follow_redirects=True) as client:
            ...
            if any(k in content_type for k in ["gbk", "gb2312", "gb18030"]):
                try:
    ...
    def _parse_metadata(self, soup: BeautifulSoup, url: str) -> Dict[str, Any]:
        ...
        if author_el:
            if author_el.name == "meta":
                author = author_el.get("content", "Unknown Author").strip()
            else:
        ...
        if author:
            ...
            if len(author) > 50 or any(kw in author for kw in ["首页", "书架", "排行", "登录", "注册", "NTR", "分类", "经典", "本日排行"]):
                author = "Khuyết Danh"
    ...
    async def get_chapter_content(self, url: str) -> str:
        ...
        if not content or len(content.strip()) < 150:
            logger.info(f"⚠️ Chapter content too short using static parser. Trying Playwright...")
            try:
                html = await self._fetch_html_playwright(url)
                soup = BeautifulSoup(html, "lxml")
                content = self._parse_chapter_content(soup)
    ...
    def _parse_chapter_content(self, soup: BeautifulSoup) -> str:
        selectors = [
            ...
            "#txtcontent", ".txtcontent", ".txtnav", ".txt_cont",
            "#chapter_content", ".read-content", ".book_content",
            "#htmlContent", "#contents", ".showtxt", "#nr1", "#nr", ".nr",
            ".box_box", ".contentbox", ".main-text", "#text", ".text", ".p"
        ]
