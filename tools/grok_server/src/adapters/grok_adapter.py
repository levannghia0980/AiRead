...
class GrokAdapter(BaseAdapter):
    """
    Production Adapter for Grok Web interface.
    """
    ...
    async def connect(self) -> bool:
        logger.info("[GrokAdapter] Connecting to Grok Web...")
        return await self.controller.open_chat()

    async def send_prompt(self, text: str) -> bool:
        logger.info("[GrokAdapter] Sending prompt to Grok...")
    ...
    async def wait_response(self, timeout: float = 120.0, stability_wait: float = 3.0) -> Optional[str]:
        logger.info(f"[GrokAdapter] Waiting for Grok response (Timeout: {timeout}s)...")
    ...
    async def new_chat(self) -> bool:
        logger.info("[GrokAdapter] Starting new Grok chat...")
    ...
    async def health_check(self) -> bool:
        try:
            ...
            logger.debug(f"[GrokAdapter] Health check passed. URL: {url} | Title: {title}")
            return True
        except Exception as e:
            logger.warning(f"[GrokAdapter] Health check failed: {e}")
            return False
